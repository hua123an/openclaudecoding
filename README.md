...
│   ├── adapters/            # CLI 工具适配器
│   │   ├── base.ts          # 适配器接口定义（含 StreamEventResult）
│   │   ├── generic.ts       # 通用适配器实现
│   │   └── index.ts         # 各工具配置 + stream 解析器
│   ├── ipc/handlers.ts      # IPC 消息处理（含 tool-use 事件转发）
│   ├── services/
│   │   ├── messageRunner.ts # 进程管理 + 流式输出解析 + 工具调用追踪
...
### 工具调用追踪（Claude Code stream-json）

```
content_block_start (tool_use)  → 记录工具名 + 索引
content_block_delta (input_json) → 累积输入 JSON
content_block_stop              → 格式化显示 + 生成 exec-badge
```
